...
class Table(QDialog):
    ...
    def lookTable(self):
        ...
        self.table.setHorizontalHeaderItem(
            0, QTableWidgetItem("  NO   ".ljust(12)))
        self.table.setHorizontalHeaderItem(
            1, QTableWidgetItem("NAME".ljust(12)))
        self.table.setHorizontalHeaderItem(
            2, QTableWidgetItem("SURNAME".ljust(12)))
    ...
    def addStudent(self):
        ...
        if dialog.exec():
            button_reply = QMessageBox.question(
                self, "Warning", "Do You Want to Add Student", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if button_reply == QMessageBox.Yes:
                ...
                if name and surname:
                    db = cursor.execute(
                        "INSERT INTO student (no,name,surname) VALUES (?,?,?)", (self.noStudent, name, surname))
                    connect.commit()
                    connect.close()
                    QMessageBox.about(
                        self, "Message", "Selected Student Saved")
                    self.lookTable()
                else:
                    QMessageBox.critical(
                        self, "Warning", "Selected Student DIDN'T SAVE")
            else:
                pass
    ...
    def editStudent(self):
        ...
        if dialog.exec():
            if dialog.status == "Deleted":
                connect = sqlite3.connect("students.db")
                cursor = connect.cursor()
                db = cursor.execute('DELETE FROM student where no="%d"' % (
                    int(dialog.line_search.text())))
                connect.commit()
                connect.close()
                self.lookTable()
                QMessageBox.information(
                    self, "Message", "Selected Student Deleted")
            elif dialog.status == "Updated":
                connect = sqlite3.connect("students.db")
                cursor = connect.cursor()
                db = cursor.execute("UPDATE student SET name=?,surname=? where no=?", (dialog.line_name.text(
                ), dialog.line_surname.text(), int(dialog.line_search.text())))
                connect.commit()
                connect.close()
                self.lookTable()

            QMessageBox.information(
                self, "Message", "Selected Student's Information Updated")
...
class EditStudentWindow(QDialog):
    def __init__(self, parent=None):
        ...
        self.search_button = QPushButton(
            "Search Student", self, clicked=self.searchStudent)
        ...
        self.update_button = QPushButton(
            "Update", self, clicked=self.updateStudent)
        self.update_button.setGeometry(10, 150, 80, 50)

        self.delete_button = QPushButton(
            "Delete", self, clicked=self.deleteStudent)
        ...
        self.var_list = [self.line, self.label_name, self.label_surname,
                         self.line_name, self.line_surname, self.update_button, self.delete_button]
    ...
    def searchStudent(self):
        ...
        cursor = connect.cursor()
        try:
            db = cursor.execute('SELECT name,surname from student WHERE no="%d"' % (
                int(self.line_search.text())))
        except:
            self.not_found_label.setText(
                '<center><font><b>Not Found Student</b></font></center>')
            self.not_found_label.setVisible(True)
            return
