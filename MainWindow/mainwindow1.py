...
class MainWindow(QMainWindow):
    ...
    def action_process(self):
        self.align_left_action = QAction(
            "&Align Left", self, triggered=self.AlignLeft)
        self.align_left_action.setStatusTip(
            "This option aligns paragraph left")
        self.align_left_action.setCheckable(True)
        self.align_left_action.setShortcut("Alt+Z")

        self.align_center_action = QAction(
            "&Align Center", self, triggered=self.AlignCenter)
        self.align_center_action.setStatusTip(
            "This option aligns paragraph center")
        self.align_center_action.setCheckable(True)
        self.align_center_action.setShortcut("Alt+X")

        self.align_right_action = QAction(
            "&Align Center", self, triggered=self.AlignRight)
        self.align_right_action.setStatusTip(
            "This option aligns paragraph right")
        ...
        self.italic_action = QAction(
            "&Italic", self, triggered=self.EditItalic)
        self.italic_action.setStatusTip("This option edits paragraph italic")
        self.italic_action.setCheckable(True)
        self.italic_action.setShortcut("Ctrl+I")

        self.underline_action = QAction(
            "&Underline", self, triggered=self.EditUnderline)
        self.underline_action.setStatusTip(
            "This option edits paragraph underline")
        self.underline_action.setCheckable(True)
        self.underline_action.setShortcut("Ctrl+U")

        self.font_type_action = QAction(
            "&Font Type", self, triggered=self.EditFontType)
        ...
        self.color_type_action = QAction(
            "&Color Type", self, triggered=self.EditColorType)
        ...
        self.open_file_action = QAction(
            "&Open File", self, triggered=self.OpenFile)
        self.open_file_action.setStatusTip("This option opens file")
        self.open_file_action.setShortcut(QKeySequence.Open)

        self.print_file_action = QAction(
            "&Print File", self, triggered=self.PrintFile)
        self.print_file_action.setStatusTip("This option prints file")

        self.convert_pdf = QAction(
            "&Convert PDF", self, triggered=self.ConvertPdf)
        self.convert_pdf.setStatusTip("This option convert pdf file")

        self.subject_action = QAction(
            "&Subject", self, triggered=self.AddSubject)
        self.subject_action.setStatusTip("This option adds Subject")

        self.table_action = QAction(
            "&Add Table", self, triggered=self.AddTable)
    ...
    def SaveAs(self):
        ...
        selected_file = QFileDialog.getSaveFileName(
            self, app.applicationName()+"- File Save", file, "*.txt")
        if selected_file:
    ...
    def OpenFile(self):
        ...
        selected_file = QFileDialog.getOpenFileName(
            self, app.applicationName()+" - Open File", self.save_directory, "*.txt")
        print(selected_file)
        if selected_file:
    ...
    def QuestionSave(self):
        result = QMessageBox.question(self, app.applicationName(
        ), " - Selected File Not Save \n Will Save Change?", QMessageBox.Yes, QMessageBox.No)
    ...
    def AddSubject(self):
        subject = QInputDialog.getText(
            self, "Document Subject", "Subject : ", QLineEdit.Normal, "Write Here Subject")
        if subject[1] == True:
            self.main_text.setAlignment(Qt.AlignCenter)
...
class FontTypeDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.font_bold.setChecked(
            True if self.parent.main_text.fontWeight() == QFont.Bold else False)
    ...
    def accept_clicked(self):
        ...
        self.parent.main_text.setFontWeight(
            QFont.Bold if self.font_bold.isChecked() else QFont.Normal)
        QDialog.accept(self)
