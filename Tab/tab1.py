...
class MainDialog(QDialog):
    def __init__(self, data, parent=None):
        ...
        delete_image_action = QAction(
            "Delete Image", self, triggered=self.delete_image)
        change_image_action = QAction(
            "Change Image", self, triggered=self.change_image)
    ...
    def add_eposta(self):
        selected = QInputDialog.getText(
            self, "E-Posta", "E-Posta Adresses", QLineEdit.Normal)
        if selected[1] == True:
            eposta = selected[0]
            if eposta.find("@") > -1:
    ...
    def change_image(self):
        selected_image = QFileDialog.getOpenFileName(
            self, "Selected Image", "", "*.png\n*.jpg\n*.gif")
